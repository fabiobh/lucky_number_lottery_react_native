  ...
  const handleDrawNumber = () => {
    ...
    if (availableNumbers.length > 0) {
      ...
      cards.forEach((card, index) => {
        const allNumbersDrawn = card.every(
          num => drawnNumbers.includes(num) || num === random,
        );
      ...
      <View style={styles.drawSection}>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[
              styles.drawButton,
              drawnNumbers.length === numCount && styles.drawButtonDisabled,
            ]}
            onPress={handleDrawNumber}
            disabled={drawnNumbers.length === numCount}>
            <Text style={styles.drawButtonText}>
              {drawnNumbers.length === numCount
                ? t('allNumbersDrawn')
                : t('drawNextNumber')}
            </Text>
          </TouchableOpacity>
